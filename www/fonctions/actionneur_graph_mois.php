...
$liste1 = "";
$liste2 = "";
$query9 = "SELECT SUM(actif) AS somme, date FROM `periph_".$periph['nom']."` WHERE `date` > DATE_SUB(curdate(), INTERVAL 1 MONTH) GROUP BY DATE_FORMAT(`date`, '%Y%m%d') ORDER BY DATE_FORMAT(`date`, '%Y%m%d')";
...
if(!($somme == 0)) {
$duree = 0;
$query0 = "SELECT date FROM `periph_".$periph['nom']."` WHERE actif = 1 AND  DATE_FORMAT('".$value9['date']."', '%Y%m%d') = DATE_FORMAT(`date`, '%Y%m%d') ORDER BY date";
...
Highcharts.setOptions({
    global: {
        ...
        $('#actioneur_<? echo $periph['id']; ?>-mois').highcharts({
            chart: {
            },
            title: {
                text: '<? echo $nom; ?>'
...
<div id="actioneur_<? echo $periph['id']; ?>-mois" style="width:<? echo $width; ?>;height:<? echo $height; ?>;"></div>
